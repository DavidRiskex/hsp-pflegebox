...
export async function generateOrderPdf(
  ...
  const drawCombedText = (fieldName: string, text: string, numBoxes: number, pageIndex = 0) => {
    try {
      const field = pdfForm.getTextField(fieldName);
      ...
      // Only strip dots/slashes, allow alphanumerics
      ...
      console.warn(`Could not draw combed text for field: ${fieldName}`);
    }
  };
  ...
  drawCombedText("Geburtsdatum", form.dob || "", 8, 0); // Precise comb drawing
  ...
  drawCombedText("Versicherten-Nummer", form.insuranceNo || "", 10, 0); // Precise comb drawing
